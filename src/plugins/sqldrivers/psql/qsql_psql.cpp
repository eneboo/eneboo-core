...
void QPSQLDriver::Mr_Proper()
{
 ...
 QString item2;
  ...
  if (fllarge)
  	{    
//     QString mproperMsg2(tr("� Usa m�ltiples tablas para FLLarge (modo AbanQ) ?"));
 ...
 if (qry4.value(0).toString() == "true")
  ...
  while (qry.next()) {
    ...
    if (!mtdIdx->field(fieldIdx))
      continue;
    qWarning(tr("Borrando �ndice %1").arg(qry.value(1).toString()));
...
//		     	qWarning("PASO !: A�adiendo tabla ... fllarge_" + v2.section('@', 1, 1));
		     if (multiFlLarge)
