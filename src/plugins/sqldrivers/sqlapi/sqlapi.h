...
class SqlApiDriver : public FLSqlDriver
{
  Q_OBJECT

public:

  SqlApiDriver(QObject *parent = 0, const char *name = 0);
  ~SqlApiDriver();
  ...
  QString userApi;
  QString passwordApi;
  map<int,QStringList> tablas_cacheadas;
...
private:
  ...
  QString query;
  const SqlApiDriver *driver;
  Dataset *dataSet;

  friend class SqlApiDriver;
};
