...
bool SqlApiDriver::open(const QString &db, const QString &, const QString &, const QString &, int)
{
  if (db.isEmpty()) {
#ifdef FL_DEBUG
    qWarning("SqlApiDriver::open() : No Database name");
#endif
...
QString SqlApiDriver::sqlCreateTable(const FLTableMetaData *tmd)
{
...
QString SqlApiDriver::formatValueLike(int t, const QVariant &v, const bool upper)
{
  QString res("IS NULL");
...
QString SqlApiDriver::formatValue( const QSqlField* field, bool trimStrings ) const
{
    QString r;
    if ( field->isNull() )
	r = nullText();
...
QString SqlApiDriver::formatValue(int t, const QVariant &v, const bool upper)
{
  QString res;
...
QVariant SqlApiDriver::nextSerialVal(const QString &table, const QString &field)
{
...
bool SqlApiDriver::alterTable(const QString &mtd1, const QString &mtd2, const QString &key)
{
#ifndef FL_QUICK_CLIENT
...
bool SqlApiDriver::hasFeature(QSqlDriver::DriverFeature feature) const
{
  switch (feature) {
...
QSqlIndex SqlApiDriver::primaryIndex2(const QString &tblname) const
{
...
QSqlIndex SqlApiDriver::primaryIndex(const QString &tablename) const
{
...
QSqlQuery SqlApiDriver::createQuery() const
{
  return QSqlQuery(new SqliteResult(this));
}

QSqlRecord SqlApiDriver::record2(const QString &tblname) const
{
  if (!isOpen() || !dataBase_)
    return QSqlRecord();
...
QSqlRecord SqlApiDriver::record(const FLTableMetaData *mtd) const
{
  QSqlRecord fil;
...
QSqlRecord SqlApiDriver::record(const QString &tablename) const
{
  QSqlRecord fil;
  if (!isOpen() || !dataBase_)
    return fil;
...
QSqlRecord SqlApiDriver::record(const QSqlQuery &query) const
{
...
QSqlRecordInfo SqlApiDriver::recordInfo2(const QString &tbl) const
{
  if (!isOpen() || !dataBase_)
...
QSqlRecordInfo SqlApiDriver::recordInfo(const QString &tablename) const
{
  QSqlRecordInfo info;
  if (!isOpen() || !dataBase_)
    return info;
...
QSqlRecordInfo SqlApiDriver::recordInfo(const QSqlQuery &query) const
{
  if (query.isActive() && query.driver() == this) {
    QSqlRecordInfo info;
    const SqliteResult *result = static_cast<const SqliteResult *>(query.result());
...
{
  driver = static_cast <const SqlApiDriver *>(d);
}
...
QVariant SqliteResult::data(int i)
{
  ...
  QString fieldName = dataSet->fieldName(i);
  field_value fV = dataSet->fv(fieldName);

  if (qstrcmp(fieldName, "binario") == 0) { // <- esto es un hack para guardar hexadecimal e interpretar binario.
