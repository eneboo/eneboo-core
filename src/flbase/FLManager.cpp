...
qWarning("AAA" + db_->database());
  ...
  if (db_->database().find("_cache.sqlite3db") >= 0) {
    qWarning(tr("FLManager::metadata %1 Trucaje!!!, size: %2").arg(n).arg(QVariant(cacheMetaData_->size()).toString()));
...
void FLManager::checkTablaCache(FLTableMetaData *tmd)
{
  ...
  if (!cacheMetaData_->find(tableName)) {
      ...
      // A�adimos el mdt a los mtds conocidos...
...
void FLManager::initCacheDB() {
  ...
  if (!FLSqlConnections::addDatabase("FLsqlite", fileCache, "", "","",0,"cache","")) {
    qWarning("FLManager::checkTablaCache : " + QApplication::tr("Error al a�adir la base de datos %1").arg(fileCache));
    return;
  }
}
