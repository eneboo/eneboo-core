...
FLSqlCursor::~FLSqlCursor()
{
  ...
  if (!d->transactionsOpened_.isEmpty())
  {
    ...
    rollbackOpened(
        -1, tr("Se han detectado transacciones no finalizadas en la �ltima operaci�n.\n"
...
QString FLSqlCursor::msgCheckIntegrity()
{
  ...
  {
    msg += QString::fromLatin1("\n") + tr("Buffer vac�o o no hay metadatos");
    return msg;
  }
    ...
    while ((field = it.current()) != 0)
    {
      ...
      if (fMD && s.isValid() && !s.isNull())
      {
        if (!field->relationM1())
        {
          msg += QString::fromLatin1("\n") +
                 tr(
                     "FLSqlCursor : Error en metadatos, el campo %1 tiene un campo asociado pero no existe relaci�n muchos a uno")
        ...
        if (!d->buffer_->isNull(pK) && s.isValid() && !s.isNull())
        {
          ...
          if (q.next())
            msg += QString::fromLatin1("\n") + d->metadata_->name() + QString::fromLatin1(":") + field->alias() +
                   tr(
                       " : Requiere valores �nicos, y ya hay otro registro con el valor %1 en este campo")
                       .arg(s.toString());
        }
      }
      ...
      if (field->isPrimaryKey() && d->modeAccess_ == INSERT && s.isValid() && !s.isNull())
      {
        ...
        if (q.next())
          msg += QString::fromLatin1("\n") + d->metadata_->name() + QString::fromLatin1(":") + field->alias() +
                 tr(
                     " : Es clave primaria y requiere valores �nicos, y ya hay otro registro con el valor %1 en este campo")
                     .arg(s.toString());
      }
      ...
      if (fieldListCK && !checkedCK && d->modeAccess_ == INSERT)
      {
        if (!fieldListCK->isEmpty())
        {
          ...
          if (q.next())
            msg += QString::fromLatin1("\n") + fields +
                   tr(" : Requiere valor �nico, y ya hay otro registro con el valor %1").arg(valuesFields);
          checkedCK = true;
        }
      }
    }
    ...
    while ((field = it.current()) != 0)
    {
      ...
      if (!relationList->isEmpty())
      {
        ...
        while ((r = itR.current()) != 0)
        {
          ...
          {
            msg += QString::fromLatin1("\n") +
                   tr("FLSqlCursor : Error en metadatos, %1.%2 no es v�lido.\n"
        ...
        QMessageBox::No ==
            QMessageBox::warning(
                ...
                tr("Los registros que va a modificar est�n bloqueados actualmente,\n"
                   "si continua hay riesgo de que su conexi�n quede congelada hasta "
...
void FLSqlCursor::copyRecord()
{
  ...
  if (!isValid() || size() <= 0)
  {
    QMessageBox::warning(qApp->focusWidget(),
                         tr("Aviso"),
                         tr("No hay ning�n registro seleccionado"),
                         QMessageBox::Ok, 0, 0);
    return;
  }
...
void FLSqlCursor::openFormInMode(int m, bool cont)
{
  ...
  if (d->action_->formRecord().isEmpty())
  {
    QMessageBox::warning(
        ...
        tr("No hay definido ning�n formulario para manejar "
  ...
  else
  {
    ...
    if (iface && iface->obj())
    {
      QMessageBox::warning(
          qApp->focusWidget(), tr("Aviso"),
          tr("Ya hay abierto un formulario de edici�n de registros para esta "
             "tabla.\nNo se abrir�n m�s para evitar ciclos repetitivos de "
...
{
  // --> Aulla Desactiva edici�n con doble click
...
void FLSqlCursor::rollbackOpened(int count, const QString &msg)
{
  ...
  {
#ifdef FL_DEBUG
    qWarning(tr("FLSqlCursor : Deshaciendo transacci�n abierta %1").arg(transactionLevel()));
#endif
    rollback();
  }
}
...
void FLSqlCursor::commitOpened(int count, const QString &msg)
{
  ...
  {
#ifdef FL_DEBUG
    qWarning(tr("FLSqlCursor : Terminando transacci�n abierta %1").arg(transactionLevel()));
#endif
    commit();
  }
}
