/****************************************************************************
** $Id: qsclass.cpp  1.1.5   edited 2006-12-01T14:48:18$
**
    ...
    for (QSMemberMap::ConstIterator it = mmap->begin(); it != mmap->end(); ++it) {
      if ((*it).type() == QSMember::ScriptFunction) {
        ...
        if ((*it).scriptFunction) {
      	...
      	qWarning( "QSClass::clear() ignoring %s", (*it).type() );
      }
    }
    delete mmap;
    mmap = 0;
...
QString QSClass::debugString(const QSObject *obj) const
{
  ...
  if (obj->isPrimitive()) {
    retVal = toString(obj).replace('\n', "").simplifyWhiteSpace().left(100) +
...
static inline bool compareScopes(const QSObject &a, const QSObject &b)
{
...
QSObject QSClass::invoke(QSObject *objPtr, const QSMember &mem) const
{
  ...
  switch (mem.type()) {
    ...
    case QSMember::ScriptFunction: {
      ...
      QSObject scope = scopeDef->construct(*env()->arguments());
      ...
      if (compareScopes(*objPtr, env()->currentScope())) {
        ...
        // ### AbanQ
        if (chain.size() == 0 && objPtr &&
            scopeDef->enclosingClass()->identifier() ==
            objPtr->objectType()->identifier()) {
          chain = env()->scope();
          pushObj = true;
        }
        // ### AbanQ
                             ...
                             QSMember *m) const
{
  ...
  const QSWritable *w = static_cast<const QSWritable *>(o->shVal());   //data( o );

  if (!w || !w->hasProperty(n)) {
