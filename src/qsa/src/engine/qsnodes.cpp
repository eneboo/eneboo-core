/****************************************************************************
** $Id: qsnodes.cpp  1.1.5   edited 2006-12-05T14:10:21$
**
...
{
  return (const_cast<QSNode *>(this))->lhs(env).dereference();
}
...
bool QSNode::setBreakpoint(int id, int line, bool set)
{
  return FALSE;
}
...
{
  // ### ???
  return (const_cast<QSNode *>(static_cast<const QSNode *>(this)) ->evaluate(env));
}
...
QSObject QSMultNode::rhs(QSEnv *env) const
{
  ...
  if (v1.objectType() == env->numberClass() && v2.objectType() == env->numberClass()) {
    switch (oper) {
      case '*':
        return env->createNumber(v1.dVal() * v2.dVal());
      case '/':
        return env->createNumber(v1.dVal() / v2.dVal());
      case '%':
        return env->createNumber(fmod(v1.dVal(), v2.dVal()));
      default:
        break;
    }
  }
...
QSObject QSShiftNode::rhs(QSEnv *env) const
{
  ...
  switch (oper) {
    ...
    case OpURShift:
      ...
      break;
    default:
      Q_ASSERT(!"ShiftNode: unhandled switch case");
      result = 0L;
  }
...
QSNODE_REF_IMPL_3(QSElementNode, list, elision, node)
