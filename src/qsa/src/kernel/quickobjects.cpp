/****************************************************************************
** $Id: quickobjects.cpp  1.1.5   edited 2006-12-06T14:03:38$
**
...
static inline QSObject uObjectToQS(QuickInterpreter *ip,
                                   QUObject *o, const void *extra = 0, QObject *qobj = 0);
static inline bool qsToUObject(QUObject *o,
                               const QSObject &v,
                               QUType *t,
                               const void *extra,
...
//### AbanQ
static QMap<QString, QValueList<QuickMetaData> > aqMetaDataCache_;

static inline QValueList<QuickMetaData> getSlots(QObject *o, const char *s, bool super)
{
  //### AbanQ
  QString key(QString::fromLatin1(s) + o->className());
  QMap<QString, QValueList<QuickMetaData> >::const_iterator it(
    aqMetaDataCache_.find(key)
  );
  if (it != aqMetaDataCache_.end()) {
#if 0
    printf("HIT aqMetaDataCache_   %s %s %d\n", key.latin1(), o->name(), aqMetaDataCache_.size());
#endif
    return *it;
  }
  ...
  //### AbanQ
  if (aqMetaDataCache_.size() > 512) {
    int n = 128;
    while (--n)
      aqMetaDataCache_.erase(aqMetaDataCache_.begin());
  }
  aqMetaDataCache_.insert(key, mds);
...
static inline QObject *objectChild(QObject *wid, const char *objName,
                                   const char *inheritsClass, bool recursiveSearch)
{
  if (!wid->children())
    return 0;
...
static inline QString variantDebugString(QVariant *var)
{
  switch (var->type()) {
    case QVariant::Bool:
      return var->toBool()
             ? QString::fromLatin1("true:Boolean")
             : QString::fromLatin1("false:Boolean");
    case QVariant::Int:
    case QVariant::UInt:
    ...
    case QVariant::Font: {
      ...
      return QString::fromLatin1(
               "{family=%1:String,pointSize=%2:Number,bold=%3:Boolean,italic=%4:Boolean,underline=%5:Boolean}:Font"
    ...
    case QVariant::Map: {
      ...
      for (QMap<QString, QVariant>::ConstIterator it = map.begin();
        ...
        str += it.key() + QString::fromLatin1("=") + (*it).toString() +
               QString::fromLatin1(":") + QString::fromLatin1((*it).typeName());
        first = FALSE;
      }
...
//### AbanQ
static QMap<QString, int> aqSlotsCache_;

static inline QString aqMethodSignature(QSEnv *env, QObject *qobj,
                                        const QUMethod *m)
{
  Q_ASSERT(m);

  bool isRet = m->count && m->parameters[0].inOut == QUParameter::Out;
  QString ret("%1:%2%3");
  ret = ret.arg(isRet ? m->parameters[0].type->desc() : "0")
        .arg(qobj->className())
        .arg(m->name);

  const QSList &args = *env->arguments();
  for (QSListIterator it = args.begin(); it != args.end(); ++it)
    ret += (*it).typeName();

  return ret;
}

static inline int aqSeekCacheSlot(const QString &mSig,
                                  QValueList<QuickMetaData>::ConstIterator &md)
{
  QMap<QString, int>::const_iterator it(aqSlotsCache_.find(mSig));
  if (it != aqSlotsCache_.end()) {
    int mdi = *it;
    while ((*md).id != mdi)
      ++md;
#if 0
    printf("HIT aqSlotsCache_   %s %d %d\n", mSig.latin1(), mdi, aqSlotsCache_.size());
#endif
    return mdi;
  }
  return -1;
...
static inline QSObject qsa_execute_slot_no_cast(QSEnv *env, QObject *qobj,
                                                ...
                                                bool *matched)
{
  ...
  for (QValueList<QuickMetaData>::ConstIterator md = mds.begin(); md != mds.end(); ++md) {
    ...
    int i = 0;
    for (QSListIterator it = args.begin(); it != args.end() && matching; ++it, ++i) {
      const QSObject &arg = (*it);
    ...
    if (matching) {
      ...
      *matched = true;
      ...
      qobj->qt_invoke((*md).id, slotargs);

      // has return value
      if (offset)
        return uObjectToQS(ip, &slotargs[0], m->parameters[0].typeExtra, qobj);
      else
        return QSObject();
    }
  }
                                   ...
                                   const QValueList<QuickMetaData> &mds)
{
  ...
  int mSigId = -1;
  if (mds.size() > 1) {
    mSig = aqMethodSignature(env, qobj, (*md).method);
    mSigId = aqSeekCacheSlot(mSig, md);
  }
  //### AbanQ
    ...
    for (int i = 0; i < m->count; i++) {
      ...
      if (!qsToUObject(&uo[ i + (firstIsReturn ? 0 : 1)],
        ...
        if (QUType::isEqual(p[ i ].type, &static_QUType_QVariant)) {
          QVariant var(args[ methodInfo.no ].toVariant((QVariant::Type) * (char *)p[i].typeExtra));
          if (var.type() != QVariant::Invalid)
            static_QUType_QVariant.set(&uo[ i + returnOffset ], var);
  ...
  //### AbanQ
  if (mSigId == -1 && mds.size() > 1) {
    aqSlotsCache_.insert(mSig, (*md).id);
#if 0
    printf("NO MATCH %s ", qobj->className());
    for (int i = 0; i < args.size(); ++i) {
      const QSObject &arg = args.at(i);
      printf(" %s ", arg.typeName().latin1());
    }
    printf("\n");
    md = mds.begin();
    while (md != mds.end()) {
...
bool QSWrapperClass::member(const QSObject *objPtr, const QString &pro, QSMember *mem) const
{
  if (!objPtr)
    return QSWritableClass::member(objPtr, pro, mem);
  ...
  // cache lookup
  QMap<QString, QSOT::QuickScriptObjectType>::ConstIterator it2 = sh->hasPropCache.find(pro);

  if (it2 != sh->hasPropCache.end()) {
    if (*it2 == QSOT::Object) {
      for (int i = objects.count() - 1; i >= 0; --i) {
        if (objectChild(objects[ i ], pro.ascii(), "QObject", FALSE))
          return TRUE;
      }
    } else if (*it2 != QSOT::Unknown) {
      QMap<QString, QuickScriptProperty>::Iterator it = sh->propertyCache.find(pro);
      ...
      if (it != sh->propertyCache.end()) {
        ...
        switch (*it2) {
          case QSOT::Property: {
            for (i = objects.count() - 1; i >= 0; --i) {
              const QMetaProperty *mp =
                ...
                property(objects[i]->metaObject()->findProperty(pro.ascii(), TRUE), TRUE);
              if (mp) {
                QVariant v;
                ...
                if (mp->isEnumType()) {
                  ...
                  sh->propertyCache.replace(
                    pro, QuickScriptProperty(QSOT::Property, var, i)
                  );
                  return TRUE;
                }
              }
          ...
          case QSOT::Slot: {
            for (i = objects.count() - 1; i >= 0; --i) {
              QValueList<QuickMetaData> mds = getSlots(objects[ i ], pro.ascii(), TRUE);
              if (!mds.isEmpty()) {
                mem->setExecutable(TRUE);
                mem->setWritable(FALSE);
                QuickScriptObjectFunc *func = 0; // new QuickScriptObjectFunc( objects[ i ], mds );
                sh->propertyCache.replace(
                  pro, QuickScriptProperty(QSOT::Slot, func, i)
                );
                return TRUE;
              }
            }
              ...
              for (i = objects.count() - 1; i >= 0; --i) {
                ...
                for (int k = 0; k < (int)enums.count(); ++k) {
                  ...
                  for (int l = 0; l < (int)me->count; ++l) {
                    if (qstrcmp(me->items[l].key, pro.ascii()) == 0) {
                      mem->setWritable(FALSE);
                      sh->propertyCache.replace(
                        pro, QuickScriptProperty(QSOT::Enum, me->items[l].value, i)
                      );
                      return TRUE;
                    }
                  }
  ...
  for (int k = 0; k < enums.count(); ++k) {
    ...
    for (int l = 0; l < me->count; ++l) {
      if (qstrcmp(me->items[l].key, pro.ascii()) == 0) {
        mem->setWritable(FALSE);
        sh->hasPropCache.replace(pro, QSOT::Property);
        sh->propertyCache.
        replace(pro, QuickScriptProperty(QSOT::Enum, me->items[l].value, 0));
        return TRUE;
      }
    }
  }
  ...
  for (int j = (int)objects.count() - 1; j >= 0; --j) {
    QObject *o = objects[ j ];
    const QMetaObject *meta = o->metaObject();
    const QMetaProperty *mp = meta->
                              property(meta->findProperty(pro.ascii(), TRUE), TRUE);
    if (mp) {
      if (!mp->scriptable()) {
        return FALSE;
      }
      ...
      if (mp->isEnumType()) {
        ...
        QuickScriptVariant *var =
          new QuickScriptVariant(interpreter(), v, mp);
        sh->propertyCache.replace(
          pro, QuickScriptProperty(QSOT::Property, var, j)
        );
        return TRUE;
      }
    }

    if (objectChild(o, pro.ascii(), "QObject", FALSE)) {
      sh->hasPropCache.replace(pro, QSOT::Object);
      return TRUE;
    }

    QValueList<QuickMetaData> mds = getSlots(o, pro.ascii(), TRUE);
    if (!mds.isEmpty()) {
      mem->setExecutable(TRUE);
      mem->setWritable(FALSE);
      QuickScriptObjectFunc *func = 0; // new QuickScriptObjectFunc( objects[ i ], mds );
      sh->propertyCache.replace(
        pro, QuickScriptProperty(QSOT::Slot, func, j)
      );
      sh->hasPropCache.replace(pro, QSOT::Slot);
      return TRUE;
    }
    ...
    for (int k = 0; k < enums.count(); ++k) {
      ...
      for (int l = 0; l < me->count; ++l) {
        if (qstrcmp(me->items[l].key, pro.ascii()) == 0) {
          sh->propertyCache.replace(
            pro, QuickScriptProperty(QSOT::Enum, me->items[l].value, j)
          );
          return TRUE;
        }
      }
  ...
  sh->hasPropCache.replace(pro, QSOT::Unknown);
...
QSObject QSWrapperClass::invoke(QSObject *objPtr, const QSMember &mem) const
{
  ...
  for (uint i = objects.count() - 1; i >= 0; --i) {
...
QVariant QSWrapperClass::toVariant(const QSObject *obj, QVariant::Type) const
{
  ...
  if (!objects.isEmpty()) {
    QObject *qo = objects[0];
    if (qo->metaObject()->findProperty("toVariant") != -1) {
      QVariant var(qo->property("toVariant"));
      if (var.isValid())
        return var;
    }
  } else
...
QString QSWrapperClass::debugString(const QSObject *obj) const
{
  ...
  for (i = 0; i < objects.count(); i++) {
    ...
    for (int j = 0; j < meta->numProperties(TRUE); ++j) {
      ...
      QVariant var(objects[i]->property(mp->name()));
      QString strVar(variantDebugString(&var));
      if (strVar.isEmpty()) {
...
public:
  ...
  static QCString normalizeSignalSlot(const char *signalSlot) {
    return QObject::normalizeSignalSlot(signalSlot);
  }
};
...
{
  return sinst.isValid()
         ? sinst.objectType()->members(&sinst)
         : QSMemberMap();
}
...
QSObject QSObjectConstructor::construct(const QSList &args) const
{
  if (type == Class) {
    ...
    for (QSListIterator it = args.begin(); it != args.end(); ++it)
      vargs.append((*it).toVariant(QVariant::Invalid));
...
QVariant QSPointerClass::toVariant(const QSObject *obj, QVariant::Type) const
{
  ...
  const QPtrVector<QObject> &objects = objectVector(obj);
  if (!objects.isEmpty()) {
    QuickPtrDispatchObject *qo = ::qt_cast<QuickPtrDispatchObject *>(objects[0]);
    if (qo && qo->isValid()) {
      QVariant::Type type = QVariant::nameToType(qo->property("name").toCString());
      if (type != QVariant::Invalid) {
        QVariant var;
        var.rawAccess(qo->pointer(), type, true);
        return var;
      }
    }
  }
...
{
  QVariant *var = variant(obj);
  QString varStr(variantDebugString(var));
...
inline QSObject uObjectToQS(QuickInterpreter *ip, QUObject *o, const void *extra,
                            QObject *qobj)
{
  if (QUType::isEqual(o->type, &static_QUType_double))
    ...
    //### AbanQ
    QSObject aqo(ip->pointerClass()->
                 wrapPointer(QVariant::typeToName((QVariant::Type) * (char *)extra),
                             static_QUType_ptr.get(o)));
    if (aqo.isValid())
      return aqo;
    //### AbanQ
...
inline bool qsToUObject(QUObject *o, const QSObject &v,
                        QUType *t,
                        const void *extra,
                        QPtrList<qs_ptr_ref> *allocs,
