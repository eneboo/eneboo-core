...
static int wait_for_data( my_socket fd, uint timeout ) {
  ...
  */
  ...
  if ( s_err ) {     /* getsockopt could succeed */
    errno = s_err;
...
static void cli_fetch_lengths( ulong *to, MYSQL_ROW column,
  ...
  for ( end = column + field_count + 1 ; column != end ; column++, to++ ) {
    ...
    if ( start )   /* Found end of prev string */
      *prev_length = ( ulong )( *column - (( char * )start ) - 1 );
    start = *column;
    prev_length = to;
  }
}
  ...
  while ( *( cp = net->read_pos ) != 254 || pkt_len >= 8 ) {
    ...
    *prev_ptr = cur;
    ...
    for ( field = 0 ; field < fields ; field++ ) {
      if (( len = ( ulong ) net_field_length( &cp ) ) == NULL_LENGTH ) {     /* null field */
        cur->data[field] = 0;
      } else {
        cur->data[field] = to;
...
static MYSQL_RES * cli_use_result( MYSQL *mysql ) {
  ...
  if ( !( result->row = ( MYSQL_ROW )
                        my_malloc( sizeof( result->row[0] ) * ( mysql->field_count + 1 ), MYF( MY_WME ) ) ) ) {    /* Ptrs: to one row */
    my_free(( gptr ) result, MYF( 0 ) );
    DBUG_RETURN( 0 );
  }
  result->fields = mysql->fields;
...
mysql_get_server_version( MYSQL *mysql ) {
  ...
  major = ( uint ) strtoul( pos, &end_pos, 10 );
  pos = end_pos + 1;
  minor = ( uint ) strtoul( pos, &end_pos, 10 );
  pos = end_pos + 1;
