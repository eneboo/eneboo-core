...
/**
 * set option to delete comments.
 *
 * @param state        true = delete, false = don't delete.
 */
void ASBeautifier::setNoComments(bool state)
{
  noComments = state;
}
...
string ASBeautifier::beautify(const string& originalLine)
{
	...
	if (!isInDefine && activeBeautifierStack != NULL && !activeBeautifierStack->empty())
	{
		...
		activeBeautifierStack->back()->noComments = noComments;
	...
	{
	  if ((!isInComment && !isInLineComment) || !noComments)
