/****************************************************************************
** $Id: qsacompletion.cpp  1.1.5   edited 2006-12-01T14:29:47$
**
                                        ...
                                        QSObject &obj)
{
  ...
  printf("QSACompletion::completeQMetaObject: %s\n", object.latin1());
#endif
...
#if defined ( QSA_COMPLETION_DEBUG )
  printf("qsa_strip_down: pos: %d  end: %d   %s -> %s\n",
         pos, end, str.latin1(), s.latin1());
#endif
  if (end < 0)
    s = qsa_strip_open_parenthesis(s, start);
#if defined ( QSA_COMPLETION_DEBUG )
  printf("qsa_strip_open_parenthesis: %s -> %s\n",
         str.latin1(), s.latin1());
#endif
  return s;
}
...
bool QSACompletion::doObjectCompletion(const QString &obj)
{
  ...
  if (o.isNull()) {
    // ### AbanQ
    QMetaObject *mo = QMetaObject::metaObject(QString::fromLatin1("AQSObject"));
    if (mo) {
      QValueList<CompletionEntry> res;
      QSObject nullObject;
      completeQMetaObject(mo, object, res, IncludeSuperClass, nullObject);
      if (!res.isEmpty()) {
        showCompletion(res);
        return true;
      }
  ...
  switch (o.type) {
    ...
    case QSCompletionObject::TQMetaObject: {
      // ### AbanQ
      QMetaObject *mo = QMetaObject::metaObject(object.latin1());
      if (!mo)
        mo = QMetaObject::metaObject(QString::fromLatin1("AQSObject"));
      if (mo)
        completeQMetaObject(mo, object, res, IncludeSuperClass, nullObject);
      // ### AbanQ
